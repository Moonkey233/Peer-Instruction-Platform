...
interface AdminMapper {
    ...
    @Select("select * from Admin where adminId = #{adminId} and isAvailable = 1")
    Admin findAdminById(@Param("adminId")Integer id)

    @Select("select * from Admin where adminName = #{adminName} and isAvailable = 1")
    Admin findAdminByName(@Param("adminName")String name)

    //不允许查看到超级管理员
    @Select("select * from Admin where adminId <> #{adminId} and adminId <> 1 and isAvailable = 1 order by adminId")
    Page<Admin> findAllAdmin(@Param("adminId")Integer id)

    @Update("update Admin set name = #{name} , phone = #{phone} where adminId = #{adminId}")
    int updateAdmin(Admin admin)

    @Update("update Admin set isAvailable = 0 where adminId = #{adminId}")
    ...
    @Select("select * from Admin where adminId <> #{adminId} and isAvailable = 1 and ( adminId like '%\${value}%' or adminName like '%\${value}%' or  name like '%\${value}%' or phone like '%\${value}%' ) order by adminId")
